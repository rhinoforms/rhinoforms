...
import com.kaicube.simpleform.serverside.InputPojo;
...
public class FormFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
		...
		if ("true".equals(servletRequest.getParameter("simpleform"))) {
	...
	private static final class HttpServletResponseWrapperA extends HttpServletResponseWrapper {
		...
		private void parseResponseAndWrite(ServletContext servletContext, HttpServletRequest request, HttpServletResponse response,
			...
			if (formFlow != null) {
				...
				Object[] sfFormNodes = documentNode.evaluateXPath("//form[@simpleform='true']");
				LOGGER.info(sfFormNodes.length + " forms found.");
				TagNode formNode = (TagNode) sfFormNodes[0];
				...
				String script = Constants.SIMPLEFORM_SCRIPT;
				jsContext.evaluateReader(scope, new InputStreamReader(servletContext.getResourceAsStream(script)), script, 1, null);

				Object[] sfScriptNodes = documentNode.evaluateXPath("//script[@simpleform='true']");
				TagNode sfScriptNode = null;
				for (Object sfScriptNodeObject : sfScriptNodes) {
					sfScriptNode = (TagNode) sfScriptNodeObject;
					StringBuffer sfScriptNodeScript = sfScriptNode.getText();
					String sfScriptNodeScriptText = sfScriptNodeScript.toString();
					LOGGER.info("sfScriptNodeScript = " + sfScriptNodeScriptText);
					jsContext.evaluateString(scope, sfScriptNodeScriptText, "<cmd>", 1, null);
				}
